...
const MultiVariantProductCard = ({ product, image }: { product: Product; image: any }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border p-4 transition-all hover:shadow-md">
      {/* Updated name prop: Removed the (${currentVariant.size}) 
          so it only shows the clean product name.
      */}
      <ProductCard
        name={product.name}
        ...
        <div className="flex flex-wrap gap-2">
          {product.variants.map((variant, index) => (
            <button
              ...
              className={`px-3 py-1 text-sm rounded-md border transition-colors font-medium ${
                selectedVariantIndex === index
                  ? "bg-[#004d43] text-white border-[#004d43]"
                  : "bg-background text-foreground hover:bg-slate-50 border-slate-200"
              }`}
            >
              {variant.size}
            </button>
